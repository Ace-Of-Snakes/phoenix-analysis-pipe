...
#off        new_name =  'CN_NLTE_L=50e3Lsun_Teff='+f'{teff:.0f}'+'_v0='+f'{v0:.0f}'+'_z='+f'{zscale:0=+6.2f}'+'_Be='+f'{eheu_Be:0=+6.2f}'
        ...
        new_name = job_name+'.ALL-IN.PHOENIX-NewEra-SESAM-COND-2023.hummel2'
...
n_MPI = -1
template = 'template.NewEraNLTE.SESAM.ALL-IN.hummel2.dqs'
file_stem = '.array.dqs'
...
# this is the 'do all in a row mode, active if argument >=0
 liste = sorted(glob.glob('nlte*FULL*PHOENIX-NewEra-SESAM-COND-2023.20'))
 if(i >= len(liste)):
    sys.exit(-1)
 else:
    filename = liste[i]
